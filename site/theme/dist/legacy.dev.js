...
(function (win) {
  "use strict"; //exposed namespace

  var respond = {};
  win.respond = respond; //define update even in native-mq-supporting browsers, to avoid errors

  respond.update = function () {}; //expose media query support flag for external use


  respond.mediaQueriesSupported = win.matchMedia && win.matchMedia("only all").matches; //if media queries are supported, exit here
  ...
  ripCSS = function ripCSS() {
    for (var i = 0; i < links.length; i++) {
      var sheet = links[i],
          href = sheet.href,
          media = sheet.media,
          isCSS = sheet.rel && sheet.rel.toLowerCase() === "stylesheet"; //only links plz and prevent re-parsing
  ...
  makeRequests = function makeRequests() {
    if (requestQueue.length) {
      ...
      ajax(thisRequest.href, function (styles) {
        ...
        parsedSheets[thisRequest.href] = true; // by wrapping recursive function call in setTimeout 
  ...
  applyMedia = function applyMedia(fromResize) {
    var name = "clientWidth",
        ...
        now = new Date().getTime(); //throttle resize calls	
  ...
  respond.update = ripCSS; //adjust on resize
